...
Windows / Euler paths
---------------------
Without overrides, data and outputs use the existing VLM_adaptation layout.
Optional VLM_DATA_ROOT contains Processed_Mapwise/ and mapwise-dataset/;
VLM_OUTPUT_ROOT contains experiment output directories. CLI --qa-json,
--image-root, --evaluation-script and --output-dir override the defaults.
Use a distinct --output-dir for each new run; reuse it with
--resume-from-checkpoint latest when resuming.

On Euler, set paths inside the container (and bind the host directories):
  VLM_DATA_ROOT=/cluster/scratch/<user>/thesis/data
  VLM_OUTPUT_ROOT=/cluster/scratch/<user>/thesis/runs
Before starting Python, configure caches separately from training outputs:
  HF_HOME=/cluster/scratch/<user>/thesis/cache/huggingface
  HF_DATASETS_CACHE=/cluster/scratch/<user>/thesis/cache/huggingface/datasets
  XDG_CACHE_HOME=/cluster/scratch/<user>/thesis/cache
  WANDB_DIR=<run-output-directory>/wandb
  WANDB_CACHE_DIR=/cluster/scratch/<user>/thesis/cache/wandb
  WANDB_DATA_DIR=/cluster/scratch/<user>/thesis/cache/wandb-data
Create these directories before launch. HF_TOKEN_PATH can keep credentials
in Home separately from HF_HOME. Existing explicit library cache overrides
must also point to scratch. --output-dir does not relocate library caches.
Archive important results outside scratch before its automatic cleanup.
"""
...
# Defaults are independent of the terminal's working directory.
# CLI paths override these defaults; relative CLI/env paths use the working directory.
VLM_ROOT = Path(__file__).resolve().parents[2]
DATA_ROOT = Path(
    os.environ.get("VLM_DATA_ROOT") or str(VLM_ROOT / "Datasets")
).expanduser().resolve()
OUTPUT_ROOT = Path(
    os.environ.get("VLM_OUTPUT_ROOT") or str(VLM_ROOT / "Training_outputs")
).expanduser().resolve()

DEFAULT_QA_JSON = (
    DATA_ROOT / "Processed_Mapwise" / "Train_Val" / "mapwise_grpo_train.json"
)
DEFAULT_IMAGE_ROOT = DATA_ROOT / "mapwise-dataset"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="QA JSON file; defaults to VLM_DATA_ROOT or the project Datasets directory.",
    )
    ...
    parser.add_argument(
        ...
        help="MapWise image directory; defaults to <data root>/mapwise-dataset.",
    )
    ...
    parser.add_argument(
        ...
        help="Evaluator Python file; defaults to the evaluator shipped with this project.",
    )
    ...
    parser.add_argument(
        ...
        help="Run directory for checkpoints, adapter and metrics (not library caches). "
             "Defaults to VLM_OUTPUT_ROOT or the project Training_outputs directory.",
    )
